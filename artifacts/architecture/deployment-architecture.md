...
```mermaid
flowchart TD
    Browser[Web Browser / React SPA] --> CDN[CDN / Static Assets]
    CDN --> TLS[SSL/TLS Termination]
...
```mermaid
flowchart LR
    Blue[Current Production / Blue] --> Green[New Deployment / Green]
    Green --> Test[Smoke Tests]
    Test -->|Pass| Switch[Switch Traffic]
    Test -->|Fail| Rollback[Rollback to Blue]
```
