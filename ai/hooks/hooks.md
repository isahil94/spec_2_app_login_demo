...
## 7. Memory Hooks
### Before Memory Read
- Purpose: Verify read scope and authorization.
- Trigger: Before memory lookup.
- Required Inputs: Memory scope, query context.
- Expected Outputs: Read authorization status.
- Validation Rules: Scope and access policy must pass.
- Success Criteria: Safe read execution.
- Failure Handling: Deny read and emit security warning.

### After Memory Read
- Purpose: Record read outcome and confidence.
- Trigger: After memory retrieval.
- Required Inputs: Retrieved entries, retrieval metadata.
- Expected Outputs: Read result summary.
- Validation Rules: Retrieved context must be coherent.
- Success Criteria: Retrieved memory is usable.
- Failure Handling: Flag stale or inconsistent memory.

### Before Memory Update
- Purpose: Check update safety and conflict risk.
- Trigger: Before memory write.
- Required Inputs: Proposed updates, current state hash/version.
- Expected Outputs: Update readiness status.
- Validation Rules: No unauthorized overwrite.
- Success Criteria: Update is safe and scoped.
- Failure Handling: Block update and request reconciliation.

### After Memory Update
- Purpose: Confirm persistence and continuity.
- Trigger: After memory write success.
- Required Inputs: Updated entries, write metadata.
- Expected Outputs: Update confirmation record.
- Validation Rules: Consistency with workflow state required.
- Success Criteria: Memory continuity preserved.
- Failure Handling: Trigger rollback or repair process.

### Memory Conflict Detected
- Purpose: Handle concurrent or contradictory memory updates.
- Trigger: Conflict detection during read/write.
- Required Inputs: Conflicting entries and versions.
- Expected Outputs: Conflict incident and resolution requirement.
- Validation Rules: Conflict details must be complete.
- Success Criteria: Deterministic conflict resolution path.
- Failure Handling: Pause dependent execution and escalate.

## 8. Event Hooks
### Before Event Publish
- Purpose: Validate event payload and metadata.
- Trigger: Before event dispatch.
- Required Inputs: Event type, payload, correlation ids.
- Expected Outputs: Publish readiness status.
- Validation Rules: Contract conformance required.
- Success Criteria: Event is valid for dispatch.
- Failure Handling: Reject event and emit validation error.

### After Event Publish
- Purpose: Confirm dispatch completion.
- Trigger: After event dispatch attempt.
- Required Inputs: Dispatch result, event id.
- Expected Outputs: Dispatch confirmation or incident record.
- Validation Rules: Delivery state must be explicit.
- Success Criteria: Publish outcome is traceable.
- Failure Handling: Trigger retry or dead-letter handling.

### Event Delivery Failed
- Purpose: Record failed delivery.
- Trigger: Delivery acknowledgment indicates failure.
- Required Inputs: Event id, failure reason, retry metadata.
- Expected Outputs: Failure record and retry recommendation.
- Validation Rules: Failure classification required.
- Success Criteria: Delivery recovery path is defined.
- Failure Handling: Apply retry policy and escalate if exhausted.

### Duplicate Event Detected
- Purpose: Detect and handle duplicate publication.
- Trigger: Duplicate idempotency key or event identity.
- Required Inputs: Duplicate event references.
- Expected Outputs: Duplicate incident record.
- Validation Rules: Deduplication check must be deterministic.
- Success Criteria: Duplicate side effects prevented.
- Failure Handling: Suppress duplicate and log governance incident.
